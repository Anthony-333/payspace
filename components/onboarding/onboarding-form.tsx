...
export function OnboardingForm() {
  ...
  const form = useForm<Values>({
    ...
    defaultValues: { name: "", slug: "", businessType: "cafe", taxRate: 12, pricesIncludeTax: true },
  });
  const { errors, isSubmitting } = form.formState;
  const businessType = useWatch({ control: form.control, name: "businessType" });

  const onSubmit = form.handleSubmit(async ({ taxRate, ...values }) => {
    setError(null);
    try {
      const { slug } = await createShop({ ...values, taxRateBps: Math.round(taxRate * 100) });
      router.replace(`/${slug}`);
    } catch (err) {
