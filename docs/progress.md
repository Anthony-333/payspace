...
**Payment photos, asked for directly (2026-09-26).** At the till, any payment (cash, e-wallet or card) can carry a photo, and it shows beside the receipt in the shop. Only the receipt prints.
- **Checks:** `npm test` (143 tests, 4 new), `npx tsc --noEmit`, `npm run lint` and `npm run build` pass. Deployed to the cloud dev deployment, and checked there end to end with `npx convex run --identity` as the smoke owner: a real PNG uploaded and passed checkout's type check, the staff receipt returned its URL, the public receipt had no trace of it, and reusing the photo on a second order was refused. That left sale #1 (₱250 tote bag, GCash "SMOKE1") in Smoke Cafe. **Not clicked through in a browser** (local sign-in, see "Needs you").
- **Till:** each line in the payment sheet's summary has a camera button (`components/pos/payment-photo.tsx`). On a tablet it opens the back camera through the file input (`capture="environment"`, no new dependency). The photo is shrunk to 1,280 px in the browser and uploaded straight away, and Complete sale waits for it. Tap a thumbnail to retake or remove it. Photos are optional. A typed card's photo moves with it when it's added to a split, and switching method drops it.
- **Server:** `sales.payments[].photoId`, passed to `sales.checkout`, which checks each photo is an unclaimed JPEG, PNG or WebP under 1 MB, that no two payments share one, and claims it for the shop *and the sale* in `uploads.saleId`. `sales.generatePhotoUploadUrl` is open to every role, since cashiers take payment.
- **Cleanup fix:** the daily photo cleanup deleted any claimed image no product used, so it would have wiped every payment photo after a day. It now keeps any upload tied to a sale. Photos taken for an order that was never completed are still removed after a day.
- **Receipts:** the Receipts list now opens a staff receipt at `/[shop]/receipts/[saleId]` (`components/receipt/staff-receipt.tsx`, `sales.receipt`): the customer's receipt with a "Payment photos" panel beside it and a "Customer link" button. The panel is `print:hidden`, and so are the rail and top bar, so printing gives the plain 58/80 mm receipt. Cashiers see only their own sales, as before. `sales.receipt` returns `null` rather than throwing for a bad, foreign or off-limits ID, because there's no error boundary yet.
- **The public link `/r/[token]` never shows photos:** an e-wallet screenshot usually shows the customer's name and number (Data Privacy Act). `byToken` and `receipt` share one `toReceipt()` shape, so the two can't drift.
- **Retention:** photos are kept as long as the sale. A retention setting (for example, delete after 90 days) is a candidate for Week 7's settings.
